...
function maybeAddDefaultToObject<T = any>(
  ...
  if (includeUndefinedValues || isConst) {
    ...
    const isSelfOrParentRequired = isParentRequired === undefined
...
>(
  ...
  const isSelectField = !isConstant(schema) &&
    isSelect(validator, schema, rootSchema);
  ...
  const constTakesPrecedence = schema[CONST_KEY] &&
...
): T {
  {
    ...
    const objectDefaults = Object.keys(retrievedSchema.properties || {}).reduce(
      (acc: GenericObjectType, key: string) => {
        ...
        const hasParentConst = isObject(parentConst) &&
    ...
    if (retrievedSchema.additionalProperties) {
      ...
      const additionalPropertiesSchema =
        isObject(retrievedSchema.additionalProperties)
...
>(
  ...
  const computeSkipPopulate = arrayMinItemsStateBehavior?.computeSkipPopulate ??
    (() => false);
